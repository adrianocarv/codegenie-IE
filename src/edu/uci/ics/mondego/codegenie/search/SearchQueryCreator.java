...
public class SearchQueryCreator {
	...
	public String[] formQuery(IType myTestType) {
		...
		for (Iterator it = parameters.iterator(); it.hasNext();) {
			...
			myParamFQNs = it.hasNext() ? myParamFQNs + itb.getQualifiedName() + "," : myParamFQNs + itb.getQualifiedName();
			myParamSNs = it.hasNext() ? myParamSNs + itb.getName() + "," : myParamSNs + itb.getName();
		}
		...
		try {
			//wantedPackageName = myTestType.getCompilationUnit().getPackageDeclarations()[0].getElementName();
			wantedPackageName = "";
		} catch (Exception e) {
			e.printStackTrace();
		}
