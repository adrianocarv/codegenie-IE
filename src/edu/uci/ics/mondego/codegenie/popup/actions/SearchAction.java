...
public class SearchAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		// WSGenericSearchQueryParameter gsqp = new
		// WSGenericSearchQueryParameter ();
		...
		// Shell shell = new Shell();
		// IType selection = (IType)
		// ((TreeSelection)currentSelection).getFirstElement();
		//
		// InputDialog id1 = new InputDialog(shell,
		// "Select slice",
		// "Type the name of the slice to be added to the project:",
		// "sliced1", null);
		//
	...
	private void search(String[] query) {
		...
		String queryStr = query[0] + "&nbsp;" + query[1] + "&nbsp;ret:(" + query[2] + ")" + "&nbsp;args:" + query[3] + ")";
		String pageID = "1";
		...
		try {
			System.out.println("http://" + sourcererApp + "/ws-search/search?qry=" + queryStr + "&pid=" + pageID + "&epp=" + entriesPerPage);
			ins = new URL("http://" + sourcererApp + "/ws-search/search?qry=" + keywords + "&pid=" + pageID + "&epp=" + entriesPerPage
					+ "&sig=signature").openStream();
		...
		for (SearchResultEntry sre : sr.getEntries()) {
			System.out.println(sre.getEntityId() + " | " + sre.getRank() + " | " + sre.getEntityName());
		}
